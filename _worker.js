...
function validate(data) {
  ...
  for (const field of required) {
    if (!clean(data.get(field))) return `Missing required field: ${field}`;
  }
  ...
  for (const [field, max] of Object.entries(LIMITS)) {
    if (clean(data.get(field)).length > max) return `${field} is too long.`;
  }
...
async function serveAppScript(request, env) {
  ...
  return new Response(transformed, { status: response.status, headers: new Headers(response.headers) });
}
